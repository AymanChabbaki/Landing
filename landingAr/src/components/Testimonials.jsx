        ...
        <div ref={r1} className="reveal-el mb-16">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <h2 className="font-heading font-bold text-[clamp(2.3rem,4.5vw,3.6rem)] leading-[1.12] tracking-tight text-[#002B49]">
              شركاؤنا يثقون في <span className="text-[#0159A3]">خبرتنا</span>
            </h2>
            <div className="flex items-center gap-3 pb-1">
              ...
              <span className="text-sm font-semibold text-slate-600">أكثر من 500 عميل راضٍ</span>
            </div>
          </div>
        ...
        <div className="flex overflow-x-auto sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 sm:gap-6 pb-6 sm:pb-0 snap-x snap-mandatory [-ms-overflow-style:'none'] [scrollbar-width:'none'] [&::-webkit-scrollbar]:hidden">
          {testimonials.map((t, i) => (
            <div
              key={i}
              ref={refs[i]}
              className="reveal-el group flex flex-col gap-4 w-[260px] sm:w-auto shrink-0 snap-center first:mr-4 sm:first:mr-0 last:ml-4 sm:last:ml-0"
            >
              ...
              >
                ...
                {t.video && (
                  <button
                    ...
                    className="absolute top-3 left-3 z-20 w-9 h-9 rounded-full bg-slate-900/80 backdrop-blur-md border border-white/20 flex items-center justify-center text-white shadow-lg hover:bg-slate-900 transition-all duration-200"
                    title={unmutedMap[i] ? "كتم الصوت" : "تشغيل الصوت"}
                  >
                    {unmutedMap[i] ? (
                ...
                >
                  <div className="w-14 h-14 bg-white/95 backdrop-blur-md rounded-full flex items-center justify-center shadow-xl">
                    <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-[#0159A3] mr-0.5">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-4"
                    ...
                    <div className="flex flex-col items-center gap-2">
                      <div className="w-14 h-14 rounded-full border-2 border-slate-200 bg-white flex items-center justify-center shadow-sm">
                        <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-slate-400 mr-0.5">
                          <path d="M8 5v14l11-7z" />
                        </svg>
                      </div>
                ...
                <div className={`absolute bottom-4 right-4 text-white z-10 pointer-events-none transition-opacity duration-300 ${playingMap[i] ? 'opacity-0' : 'opacity-100'}`}>
